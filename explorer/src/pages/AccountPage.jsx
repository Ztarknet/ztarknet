...
import { getAccount, getAccountTransactions, getAccountTransactionCount } from '@services/zindex/accounts';
...
function BalanceChart({ data, isLoading = false }) {
  ...
  return (
    <div className="w-full border border-[rgba(255,137,70,0.2)] rounded-2xl bg-[rgba(8,8,12,0.9)] p-6 overflow-hidden">
      <h3 className="text-sm font-mono tracking-widest uppercase text-[rgba(255,137,70,0.64)] mb-4">Balance Over Time</h3>
...
export function AccountPage({ address }) {
  ...
  const [zindexTxList, setZindexTxList] = useState([]); // Store zindex tx data for balance calc
  ...
  const calculateTxBalanceChange = (tx, accountAddress) => {
    let received = 0;
    let sent = 0;
  ...
  const calculateBalanceHistory = (currentBalance, fullTxList, accountAddress) => {
    if (!fullTxList || fullTxList.length === 0) return [];

    // Sort transactions from newest to oldest (most recent first)
    const sortedTxs = [...fullTxList].sort((a, b) =>
  ...
  useEffect(() => {
    async function fetchTransactions() {
      ...
      try {
        ...
        // Store zindex tx list for balance calculation
        setZindexTxList(txList);
        ...
        const txids = [...new Set(txList.map(tx => tx.txid))];
        ...
        const validTxs = fullTxData.filter(tx => tx !== null);
        validTxs.sort((a, b) => (b.blockheight || 0) - (a.blockheight || 0));
  ...
  if (loading) {
    return (
      <div className="max-w-container mx-auto md:px-8 px-4 pt-[120px] pb-[120px] flex-1">
        ...
        <h2 className="text-[1.8rem] font-bold mb-6 text-foreground tracking-tight skeleton-text">Loading Account...</h2>

        {/* Skeleton address display */}
        <div className="mb-8">
          <div className="flex flex-col gap-2">
            <span className="text-xs font-mono tracking-widest uppercase text-[rgba(255,137,70,0.64)] mb-2">Address</span>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-6 mb-12 min-h-[160px]">
          ...
          <StatCard label="Total Transactions" value="---" description="Loading..." isLoading={true} />
        </div>
      </div>
    );
  }
  ...
  if (!account) {
    return (
      <div className="max-w-container mx-auto md:px-8 px-4 pt-[120px] pb-[120px] flex-1">
        <div className="p-6 bg-red-600/10 border border-red-600/30 rounded-xl text-red-200 font-mono mb-6">Account not found</div>
      </div>
    );
  }
  ...
  const txCount = typeof transactionCount === 'object'
    ...
    <div className="max-w-container mx-auto px-8 pt-[120px] pb-[120px] flex-1">
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-6 mb-12 min-h-[160px]">
        <StatCard
          label="Created At"
          value={createdAt}
          description="First seen on chain"
        />
        <StatCard
      ...
      {/* Transactions List */}
      <h2 className="text-[1.8rem] font-bold mb-6 text-foreground tracking-tight">
        {transactions.length}{txCount > transactions.length ? ` of ${txCount}` : ''} Transaction{transactions.length !== 1 ? 's' : ''}
      </h2>
