...
export function useRevealOnScroll() {
  useEffect(() => {
        ...
        entries.forEach((entry: IntersectionObserverEntry) => {
          if (entry.isIntersecting) {
            ...
            requestAnimationFrame(() => {
              ...
              entry.target.addEventListener('transitionend', handleTransitionEnd as EventListener);
            });
    ...
    const observeElements = () => {
      ...
      revealTargets.forEach((target: Element, index: number) => {
        ...
        mutation.addedNodes.forEach((node: Node) => {
          if (node.nodeType === 1) {
            ...
            if (
              ...
              element.querySelector?.('.reveal-on-scroll')
            ) {
              shouldObserve = true;
            }
          }
