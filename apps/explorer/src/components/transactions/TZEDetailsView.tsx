...
export function TZEDetailsView({ tx }: TZEDetailsViewProps) {
  const [expandedProof, setExpandedProof] = useState<boolean>(false);
  const [oldStateRoot, setOldStateRoot] = useState<string | null>(null);
  const [loadingOldState, setLoadingOldState] = useState<boolean>(false);
  ...
  let tzeOutputData: TzeData | null = null;
  let spendingTzeInput: Vin | null = null;

  // Check inputs for TZE data
  if (tx.vin) {
    for (const input of tx.vin as Vin[]) {
  ...
  if (tx.vout) {
    for (const output of tx.vout as Vout[]) {
  ...
  useEffect(() => {
    async function fetchOldState() {
      ...
      try {
        ...
        const prevTx = (await getRawTransaction(spendingTzeInput.txid)) as Transaction;

        // Get the output being spent
        if (prevTx.vout && prevTx.vout.length > spendingTzeInput.vout) {
          const prevOutput = prevTx.vout[spendingTzeInput.vout];

          if (prevOutput?.scriptPubKey?.hex) {
        ...
        if (error instanceof Error) {
          console.error('Error fetching old state root:', error.message);
        } else {
  ...
  useEffect(() => {
    async function fetchVerifier() {
      ...
      try {
        ...
        if (error instanceof Error) {
          console.error('[TZE Details] Error fetching verifier:', error.message);
        } else {
      ...
      try {
        ...
        if (tzeVout !== null) {
          ...
          if (spendingInputs && Array.isArray(spendingInputs) && spendingInputs.length > 0) {
            ...
            const spendingInput = spendingInputs[0] as TZEInput;
            setNextVerifyTx(spendingInput.txid);
          }
        }
      } catch (error: unknown) {
        if (error instanceof Error) {
          console.error('[TZE Details] Error fetching next verify transaction:', error.message);
        } else {
  ...
  if (!tzeOutputData) {
    return (
      <div className="p-4 bg-red-600/10 border border-red-600/30 rounded-lg text-red-200 text-sm">
        Unable to parse TZE data
      </div>
    );
  }
  ...
  let extensionView: ReactElement | null = null;
  ...
  if (tzeId === 1) {
    ...
    if (tzeMode === 0) {
      ...
      let witness: StarkVerifyWitness | null = null;
      if (tzeInputData) {
  ...
  const TZE_BADGE_STYLES: Record<string, string> = {
  ...
  return (
    <div className="mt-0 p-0">
      <div
        className="h-px my-8 mb-6"
        style={{
          background:
            'linear-gradient(to right, transparent, rgba(255, 107, 26, 0.3), transparent)',
        }}
      />

      <div className="flex items-center gap-3 mb-5">
        <h3 className="text-base font-semibold text-accent m-0 font-mono uppercase tracking-wider">
          TZE Details
        </h3>
        <span
          className={`inline-block py-1 px-3 rounded-md text-xs font-semibold uppercase tracking-wide font-mono border ${TZE_BADGE_STYLES[typeName] || TZE_BADGE_STYLES.demo}`}
        >
          {typeName}
        </span>
      </div>
      ...
      <div className="p-5 bg-[rgba(12,13,17,0.4)] rounded-[10px] border border-[rgba(255,107,26,0.15)]">
        ...
        <div className="flex flex-col gap-2 pb-4 mb-4 border-b border-[rgba(255,107,26,0.1)]">
          ...
          {modeDescription && <p className="m-0 text-muted text-sm italic">{modeDescription}</p>}
        </div>
